...
export class LevelingService {
  ...
  private readonly userCache = new Map<
    string,
    { user: User & { toObject?: () => User }; timestamp: number }
  >();
  ...
  async addXp(
    ...
    try {
      ...
      this.logger.error(
        `Error al agregar XP para usuario ${userId}: ${error.message}`,
        error.stack
      );
      throw error;
    }
  ...
  private async assignRankRole(member: GuildMember, rankName: string): Promise<void> {
    try {
      ...
      if (!rankRole) {
        this.logger.warn(
          `Rol de rango '${rankName}' no encontrado en el servidor ${member.guild.name}`
        );
        return;
      }
  ...
  private async removePreviousRankRoles(
    member: GuildMember,
    ...
    try {
      ...
      const rolesToRemove = member.roles.cache.filter(
        (role) => allRankRoles.includes(role.name) && role.name !== currentRankName
      );

      if (rolesToRemove.size > 0) {
        await member.roles.remove(rolesToRemove);
        this.logger.log(
          `Roles anteriores removidos de ${member.user.tag}: ${rolesToRemove.map((r) => r.name).join(', ')}`
        );
      }
    } catch (error) {
      this.logger.error(
        `Error al remover roles anteriores de ${member.user.tag}: ${error.message}`
      );
    }
  }
  ...
  async getProfile(userId: string): Promise<any> {
    try {
      ...
      return await this.calculateProfileData(user as unknown as User);
    } catch (error) {
      this.logger.error(
        `Error al obtener perfil de usuario ${userId}: ${error.message}`,
        error.stack
      );
      throw error;
    }
  ...
  private async calculateProfileData(user: User): Promise<any> {
    try {
      ...
      this.logger.error(
        `Error al calcular datos del perfil para usuario ${user.discordId}: ${error.message}`
      );
      throw error;
    }
  }
  ...
  async resetUserLevel(userId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Error al resetear nivel de usuario ${userId}: ${error.message}`,
        error.stack
      );
      throw error;
    }
  ...
  async getUserStats(userId: string): Promise<any> {
    try {
      ...
      this.logger.error(
        `Error al obtener estadísticas de usuario ${userId}: ${error.message}`,
        error.stack
      );
      throw error;
    }
  ...
  private async calculateRank(userId: string): Promise<number> {
    try {
      ...
      const higherUsers = await this.userModel.countDocuments({
        $or: [{ level: { $gt: user.level } }, { level: user.level, xp: { $gt: user.xp } }],
      });
