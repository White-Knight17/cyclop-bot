...
import { Client, Guild, PermissionFlagsBits, ColorResolvable } from 'discord.js';
  ...
  private getNextRankInfo(currentLevel: number) {
    ...
    for (const rank of RankConfig.ranks) {
      if (currentLevel < accumulatedLevels + rank.levels) {
        ...
        const nextRank =
          nextLevel > accumulatedLevels + rank.levels
        ...
        for (let level = rank.levels; level >= 1; level--) {
          ...
          if (!guild.roles.cache.some((r) => r.name === roleName)) {
            try {
              ...
              const position =
                basePosition - (RankConfig.ranks.length - i) * rank.levels - (rank.levels - level);
