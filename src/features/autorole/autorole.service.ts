...
export class AutoRoleService {
    ...
    async setAutoRole(guildId: string, roleId: string): Promise<AutoRole> {
        try {
            const autoRole = await this.autoRoleModel.findOneAndUpdate(
                { guildId },
    ...
    async disableAutoRole(guildId: string): Promise<void> {
        try {
            await this.autoRoleModel.findOneAndUpdate(
                { guildId },
                { enabled: false }
            ).exec();
    ...
    async validateAndAssignRole(member: GuildMember, roleId: string): Promise<boolean> {
        try {
            ...
            if (role.position >= botMember.roles.highest.position) {
                this.logger.warn(`Rol ${role.name} está por encima del rol más alto del bot en ${member.guild.name}`);
                return false;
            }

            // Asignar el rol
            await member.roles.add(role);
            this.logger.log(`Rol ${role.name} asignado exitosamente a ${member.user.tag} en ${member.guild.name}`);
            return true;
