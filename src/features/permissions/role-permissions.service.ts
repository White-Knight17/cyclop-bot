...
export class RolePermissionsService {
    ...
    async setAdminRoles(guildId: string, roleIds: string[]): Promise<RolePermissions> {
        try {
            const config = await this.rolePermissionsModel.findOneAndUpdate(
                { guildId },
                {
                    guildId,
                    adminRoles: roleIds,
            ...
            this.logger.error(`Error al actualizar roles de administrador para ${guildId}: ${error.message}`);
            throw error;
        }
    }
    ...
    async getAdminRoles(guildId: string): Promise<string[]> {
        try {
            ...
            this.logger.error(`Error al obtener roles de administrador para ${guildId}: ${error.message}`);
            return [];
        }
    }
    ...
    async disableRolePermissions(guildId: string): Promise<void> {
        try {
            await this.rolePermissionsModel.findOneAndUpdate(
                { guildId },
                { enabled: false }
            ).exec();
            this.logger.log(`Permisos de roles deshabilitados para ${guildId}`);
        } catch (error) {
            this.logger.error(`Error al deshabilitar permisos de roles para ${guildId}: ${error.message}`);
            throw error;
        }
    }
} 