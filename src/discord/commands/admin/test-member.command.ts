...
export class TestMemberCommand {
  ...
  async simulateMember(@Context() [interaction]: SlashCommandContext) {
    ...
    try {
      ...
      if (!intents.has('GuildMembers')) {
        return interaction.reply({
          content:
            '❌ El bot no tiene el intent GuildMembers habilitado. Por favor, habilítalo en el portal de desarrolladores de Discord.',
          ephemeral: true,
        });
      }
