    ...
    try {
      for (const rank of RankConfig.ranks) {
        for (let level = 1; level <= rank.levels; level++) {
          ...
          const existingRole = guild.roles.cache.find((r: Role) => r.name === roleName);
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: 'Roles Creados',
            value: createdRoles.length > 0 ? createdRoles.join('\n') : 'Ninguno (ya existían)',
            inline: true,
          },
          {
            name: 'Roles Existentes',
            value: skippedRoles.length > 0 ? skippedRoles.join('\n') : 'Ninguno',
            inline: true,
          }
        );
