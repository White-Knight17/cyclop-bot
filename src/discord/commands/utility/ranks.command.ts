...
export const RanksCommandDecorator = createCommandGroupDecorator({
  ...
  nameLocalizations: {
    // Localizaciones para soporte multi-idioma
    'en-US': 'ranks',
    'es-ES': 'rangos',
  },
...
export class RanksCommand {
  ...
  public async actualRank(@Context() [interaction]: [ChatInputCommandInteraction]) {
  ...
  public async profileRank(@Context() [interaction]: [ChatInputCommandInteraction]) {
  ...
  private getProgressBar(
    ...
    const progress = Math.min(
      1,
      Math.max(0, (safeCurrent - safeCurrentLevel) / (safeNextLevel - safeCurrentLevel))
    );
  ...
  async leaderboard(@Context() [interaction]: [CommandInteraction]) {
    ...
    const embed = new EmbedBuilder().setTitle('🏆 Leaderboard del Servidor').setColor('#FFD700');
  ...
  public async top(@Context() [interaction]: [ChatInputCommandInteraction]) {
    try {
