...
export class MessageCreateEvent {
  ...
  async onMessageCreate([message]: [Message]): Promise<void> {
    try {
      ...
      this.logger.error(
        `Error procesando mensaje de ${message.author?.tag || 'usuario desconocido'}: ${error.message}`,
        error.stack
      );
    }
  }
  ...
  private async handleLevelUp(message: Message, result: any): Promise<void> {
    try {
      ...
      this.logger.log(
        `🎉 ${message.author.tag} subió al nivel ${result.newLevel} en ${message.guild?.name}`
      );
    } catch (replyError) {
      this.logger.warn(
        `No se pudo enviar mensaje de subida de nivel para ${message.author.tag}: ${replyError.message}`
      );
      ...
      try {
        ...
        this.logger.error(
          `Error en fallback de mensaje de subida de nivel: ${fallbackError.message}`
        );
      }
    }
  }
  ...
  private cleanupExpiredCooldowns(): void {
    ...
    if (process.env.NODE_ENV === 'development') {
      this.logger.debug(
        `Limpieza de cooldowns completada. Cooldowns activos: ${this.userCooldowns.size}`
      );
    }
  }
